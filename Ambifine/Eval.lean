...
partial def eval [Monad m] [MonadError m] (ρ : Env) : Term → m Term
  ...
  | Term.pair l r    => Term.pair (eval ρ l) (eval ρ r)
  | Term.repr l r    => Term.repr (eval ρ l) (eval ρ r)
  | Term.elem x p A  => Term.elem (eval ρ x) p A
  | Term.inj b A t   => Term.inj b A (eval ρ t)
  | Term.em A        => Term.em A
  | Term.cons x xs   => Term.cons (eval ρ x) (eval ρ xs)

  -- ── β-reduction: function / proof / irrelevant application ───────────────
  | Term.app A f x =>
    let xv := eval ρ x
    match eval ρ f with
    | Term.lam _ body => eval ρ (body.subst0 xv)
    | fv              => Term.app A fv xv
  | Term.app_pr A f p =>
    match eval ρ f with
    | Term.lam_pr _ body => eval ρ (body.subst0 p)
    | fv                 => Term.app_pr A fv p
  | Term.app_irrel A f x =>
    match eval ρ f with
    | Term.lam_irrel _ body => eval ρ (body.subst0 x)
    | fv                    => Term.app_irrel A fv x

  -- ── Σ / set / union elimination ──────────────────────────────────────────
  | Term.let_pair k P e e' =>
    match eval ρ e with
    | Term.pair l r => eval ρ ((e'.subst0 r).subst0 l)
    | ev            => Term.let_pair k P ev e'
  | Term.let_set k P e e' =>
    match eval ρ e with
    | Term.elem x p _ => eval ρ ((e'.subst0 p).subst0 x)
    | ev              => Term.let_set k P ev e'
  | Term.let_repr k P e e' =>
    match eval ρ e with
    | Term.repr l r => eval ρ ((e'.subst0 r).subst0 l)
    | ev            => Term.let_repr k P ev e'

  -- ── Coproduct elimination ────────────────────────────────────────────────
  | Term.case k K d l r =>
    match eval ρ d with
    | Term.inj 0 _ x => eval ρ (l.subst0 x)
    | Term.inj 1 _ x => eval ρ (r.subst0 x)
  ...
  | Term.listrec k K e nil_case cons_case =>
    ...
    | Term.cons x xs =>
      ...
      eval ρ (((cons_case.subst0 ih).subst0 xs).subst0 x)
